...
info = {
    "year": 2023,
    "version": 'v1',
    "description": "COCO FDG PET MIP Pediatric Lymphoma Detection",
    "contributor": ["Stanford University, USA", "IBM Research, USA"],
#     "url": "",
...
source_license = {
    "id": 1,
    "name": "JSA between Stanford and IBM Research",
    "url": ""
}
...
def find_studies(path_to_data,keys):
    # find all studies
    root = plb.Path(path_to_data)
    study_paths = list(root.glob('{}_*{}*.nii.gz'.format(keys[0],keys[1])))
    return study_paths
...
def get_annotations_images_per_nifti(data, pet_path, sampled_frames, category_id, images, image_id, annotations, annotation_id, image_out_root):
    # "images" info: w and h same for frames from same nifti
    w, h = data[:,:,0].shape
    
    # get study information
    diagnosis = None #row['diagnosis']
    age = None #row['age']
    sex = None #row['sex']
