...
def images_annotations_info(data_in_root, table, num_frames, image_id, annotation_id, image_out_root):
    ...
    for i, row in tqdm(table.iterrows(), total=table.shape[0]):
        ...
        if num_frames > data.shape[-1]:
            ...
            images_per_nifti, image_id, annotations_per_nifti, annotation_id = get_annotations_images_per_nifti(data, row, sampled_frames
                                                        , category_id, images, image_id, annotations, annotation_id, image_out_root)
            images.extend(images_per_nifti)
...
if __name__ == "__main__":
    ...
    for keyword, table in zip(["train", "test"],[train_tab,test_tab]):
        ...
        with open(os.path.join(data_out_root,'annotations',"detr_{}.json".format(keyword)),"w") as outfile:
            json.dump(coco_format, outfile, cls=NpEncode)
...
# python src/create_annotations_petct_detr.py /gpfs/fs0/data/stanford_data/petct/NIFTI_MIP/FDG-PET-CT-Lesions/ /gpfs/fs0/data/stanford_data/petct/DETR_MIP/FDG-PET-CT-Lesions/


# Output dataset directory struction:
