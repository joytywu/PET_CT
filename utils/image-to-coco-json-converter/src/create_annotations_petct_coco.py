...
def get_annotations_images_per_nifti(data,study_path, row,image_id, sampled_frames, category_id, annotation_id, annotations):
    ...
    for frame_idx in sampled_frames:
        ...
        image = create_image_annotation(study_path, w, h, image_id, frame_idx, age, sex)
        images.append(image)
...
def create_image_annotation(file_name, width, height, image_id, frame_idx, age, sex):
    image = {
        "file_name": file_name,
        "frame_idx": frame_idx,
        "height": height,
...
def images_annotations_info(data_in_root, nii_fname, table, num_frames, image_id, annotation_id):
    category_id = 1 # just tumor for this dataset
    annotations = [] # list of dictionaries
    images = [] # list of dictionaries
    
    #study_dirs = [os.path.join(data_in_root, rel_path) for rel_path in table['relative_path'].tolist()]
    ...
    for i, row in tqdm(table.iterrows(), total=table.shape[0]):
        ...
        study_path = os.path.join(data_in_root, row['relative_path'],nii_fname)
        ...
        if num_frames > data.shape[-1]:
            ...
            annotations_per_nifti, annotation_id, image_dict, image_id = get_annotations_images_per_nifti(data, study_path, row, image_id
...
#         # For multimodal work - need to think how to save the images so can still augment them in way that makes sense. 
...
#         # Set out paths to write out images
#         image_out_path = os.path.join(data_out_root,'images',keyword)
        
        # Create images and annotations sections
        coco_format["images"], image_id, coco_format["annotations"], annotation_id = images_annotations_info(data_in_root, nii_fname
                                                                                     , table, num_frames, image_id, annotation_id)

        with open(os.path.join(data_out_root,'annotations',"{}.json".format(keyword)),"w") as outfile:
...
# python src/create_annotations_petct.py /media/storage/Joy/datasets/NIFTI_MIP/FDG-PET-CT-Lesions/ /media/storage/Joy/datasets/cocoDataset/FDG-PET-CT-Lesions/